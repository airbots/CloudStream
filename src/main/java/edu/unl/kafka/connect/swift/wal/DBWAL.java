...
import edu.unl.kafka.connect.swift.SwiftSinkConnectorConfig;
...
public class DBWAL implements  WAL {
  ...
  public DBWAL(String logsDir, TopicPartition topicPartition, Storage storage, HdfsSinkConnectorConfig config) {
    ...
    try {
      String name = config.getString(SwiftSinkConnectorConfig.NAME_CONFIG);
      tableName = name + "_" + topicPartition.topic() + "_" + partitionId;

      String connectionURL = config.getString(SwiftSinkConnectorConfig.DB_CONNECTION_URL_CONFIG);
      String user = config.getString(SwiftSinkConnectorConfig.DB_USER_CONFIG);
      String password = config.getString(SwiftSinkConnectorConfig.DB_PASSWORD_CONFIG);
